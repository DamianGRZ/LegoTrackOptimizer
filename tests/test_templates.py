...
class TestBranchPieceComputation:
    ...
    def test_piece_count_function(self):
        """compute_branch_piece_count should match actual piece count."""
        for n in range(0, 8):
            pieces = compute_branch_pieces(LEFT_SIDING, n_straights=n)
            assert len(pieces) == compute_branch_piece_count(n)
