...
class TestMainLoopDecoding:
    ...
    def test_main_loop_respects_inventory(self, catalog):
        """Decoder should not exceed inventory limits."""
        inventory = {"R40_LEFT": 8, "STRAIGHT_16": 4}
        total = sum(inventory.values())
        dims = compute_dimensions(total)

        # Request more R40_LEFT than available
        pattern = [R40_LEFT] * 20
        chromosome = create_chromosome_from_pieces(dims, pattern)

        layout = decode_chromosome(chromosome, catalog, inventory, dims=dims)

        assert layout.n_pieces <= total
