...
**`src/visualization/`** — `track_renderer.py` (`plot_layout()` — the sole renderer: one full-size track view plus a metrics/legend info panel, closure tolerances required as keyword args so the drift overlay is gated by the optimizer's own thresholds; piece geometry drawn via the shared `_draw_piece`/`_draw_piece_sequence` helpers); `pareto_plot.py` (`plot_pareto_front()` built on pymoo `Scatter`); `objective_progress.py` (`load_score_progress()` + `plot_score_progress()` — F[0] per generation against the inventory ceiling; pymoo's `RunningMetricAnimation` plots front movement and needs `save_history`, so it does not answer this question).
