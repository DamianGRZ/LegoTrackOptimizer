...
> **Implementation note.** The optimizer's active physics live in `src/train.py`
> (`TrainConfig` + `v_eff_array`), which is the single source of truth for
> lateral-stability speed caps and is written to be portable (stdlib + numpy +
> yaml only, zero project imports). The implementation diverges from the
> defaults quoted below on two points:
>
> 1. **`v_motor_max = 1.10 m/s`** is used in `TrainConfig`, not the 1.57 m/s
>    quoted here. The 1.57 m/s figure is the no-load motor top speed; 1.10 m/s
>    reflects the realistic drive-train cap under load.
> 2. **No safety factor is applied to `v_slide = sqrt(mu*g*R)`.** The stability
>    margin is provided by the tip-over and Nadal wheel-climb secondary caps
>    (`TrainConfig.v_tip`, `TrainConfig.v_nadal`) and by the motor cap itself,
>    rather than by multiplying `v_slide` by 0.8.
>
> See also `docs/Lateral stability model for LEGO train layout optimization.md`
> for the derivation of these choices.
