...
*(No commit step — the user controls commit timing. Leave `tests/test_train.py` modified in the working tree and move on.)*
...
*(No commit step — the user controls commit timing. Leave `src/train.py` modified in the working tree and move on.)*
...
*(No commit step — the user controls commit timing. Leave `tests/test_data.py` modified in the working tree and move on.)*
...
*(No commit step — the user controls commit timing. Leave `src/data.py` modified in the working tree and move on.)*
...
*(No commit step — the user controls commit timing. Leave the four config YAMLs modified in the working tree and move on.)*
...
*(No commit step — the user controls commit timing. Leave all five files modified in the working tree and move on.)*
...
- [ ] **Step 2: Remove the file from disk**

```bash
rm configs/trains/with_car.yaml
```

Plain `rm`, not `git rm` — the user controls all git staging. This leaves the file as a pending deletion in the working tree; `git status` will show it with a `D` status. The user will decide how to stage and commit the deletion later. This is the only destructive filesystem action in the plan and it is authorised by the spec's "Delete configs/trains/with_car.yaml" line under "Config YAML changes".
...
*(No commit step — the user controls commit timing. Leave `configs/trains/with_car.yaml` as a working-tree deletion and move on.)*
...
- [ ] **Step 8: Record implementation observations (optional, no commit)**

If any literal outputs from Steps 1, 2, 3, or 5 revealed a divergence from expectations that the plan did not predict, append a new "Implementation observations" section to `docs/superpowers/specs/2026-04-11-train-consolidation-design.md`. Do NOT commit — the user controls all commit timing. Leave the spec file as a pending modification.

If there are no observations worth recording, skip this step entirely.

---

## Summary of working-tree changes this plan produces

Since the user controls commit timing, this plan leaves a cumulative set of modifications in the working tree rather than a sequence of commits. At the end of Task 8 the working tree contains the following changes relative to the pre-plan state:

**New files:**
- `tests/test_train.py` — TrainConfig test suite
- A new `TestStudMm` class appended to `tests/test_data.py` (modification, not a new file)

**Modified files:**
- `src/train.py` — expanded with `mu_nominal`, `mu_design`, `max_accel`, `brake_decel`, formulas flipped to `mu_design`
- `src/data.py` — `TrackCatalog.stud_mm` attribute populated from `metadata.stud_mm`
- `src/evaluation.py` — `compute_speed_profile` signature drops `physics`, reads `catalog.stud_mm` and `train_config.{max_accel,brake_decel}`
- `src/config.py` — `PhysicsConfig` class and `physics` field deleted
- `src/problem.py` — `compute_speed_profile` call no longer passes `self.config.physics`
- `tests/conftest.py` — `physics` fixture renamed to `train_config`, returns `TrainConfig()`
- `tests/test_evaluation.py` — six fixture parameter renames, R40 numeric assertion tightening, `test_utilization_range` rewrite
- `configs/default.yaml`, `configs/compact.yaml`, `configs/with_switches.yaml`, `configs/with_crossing.yaml` — `physics:` blocks deleted

**Deleted files:**
- `configs/trains/with_car.yaml` — obsolete scaffolding

**Possibly modified:**
- `docs/superpowers/specs/2026-04-11-train-consolidation-design.md` — only if Task 8 Step 8 records observations

The test suite should be green at the end of the plan with the entire set of changes applied. The user decides how to stage, split, squash, or commit these changes.
