...
#      left vs right turn direction).
#   2. R40_SWITCH_LEFT_IN/_OUT → R40_SWITCH_LEFT and same for right (one
#      physical SKU per handedness; the rotate bit selects IN vs OUT
#      placement, since OUT is just the same brick rotated 180° in plane).
# Switches remain handed (left vs right are chiral, distinct SKUs).
...
class TrackCatalog:
    ...
    def __init__(self) -> None:
        ...
        self._spec: Optional[TrackCatalogSpec] = None
        # V2 spec retained when loaded from V2 yaml; None for V1 yaml loads.
    ...
    def _from_v2_spec(cls, spec: TrackCatalogSpec) -> "TrackCatalog":
        ...
        catalog._spec = spec  # retain V2 spec for downstream port/route lookups
    ...
    def get_speed_for_route(
        ...
        """Per-route speed limit lookup (Rule 35, Rule 21).

        Single-route pieces (straights, curves) ignore ``route_name`` and
        return the default ``speed_table`` entry. Multi-route pieces
        (switches, crossings, double-crossover) consult the per-route
        physics override stored at module level. Unknown route names fall
        back to the default speed.

        DEPRECATED: returns catalog-static values disconnected from
        ``TrainConfig``. Prefer ``get_radius_m_for_route`` + ``train_config.v_eff``
        (see ``problem.py:_evaluate``). Kept for backward compatibility
        with V1-era callers; will be removed in Tier 2 cleanup.
        """
        if not 0 <= piece_idx < len(self._speed_table):
            return self.DEFAULT_SPEED
        piece_id = self._index_to_piece.get(piece_idx)
        if piece_id is None:
            return float(self._speed_table[piece_idx])
        per_route = _V2_ROUTE_PHYSICS.get(piece_id.id)
        if per_route is None or route_name not in per_route:
            return float(self._speed_table[piece_idx])
        _radius_mm, speed_limit = per_route[route_name]
        return float(speed_limit)

    def get_radius_m_for_route(
        self, piece_idx: int, route_name: str,
    ) -> Optional[float]:
        """Geometric curve radius for a (piece, route) pair, in metres.

        Returns ``None`` for straight routes (no curvature limit; train is
        motor-bound only). Distinct from :meth:`get_speed_for_route` — this
        returns geometry; speed comes from ``TrainConfig.v_eff(radius_m)``.

        Lookup order: ``_V2_ROUTE_PHYSICS`` (multi-route pieces) →
        ``_V2_DEFAULT_PHYSICS`` (curves) → fallback ``None``.
        """
        if not 0 <= piece_idx < len(self._speed_table):
            return None
        ...
        if piece is None:
            return None
        per_route = _V2_ROUTE_PHYSICS.get(piece.id)
        if per_route is not None and route_name in per_route:
            radius_mm, _speed = per_route[route_name]
        else:
    ...
    @property
    def spec(self) -> Optional[TrackCatalogSpec]:
        """V2 spec when loaded from V2 yaml; None for V1 yaml loads."""
        return self._spec
