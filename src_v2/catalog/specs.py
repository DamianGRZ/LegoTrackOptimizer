...
class TrackPieceSpec(BaseModel):
    ...
    # turn directions (curves and straights). False for chiral parts whose
    # mirror image is a distinct physical SKU (LEGO/4DBrix switches), and
    # for pieces where the flip would be redundant or misleading
    # (crossings, double crossover). Default is conservative: assume
    # asymmetric unless proven otherwise.
    symmetric: bool = False

    # Whether the physical brick can be rotated 180° in plane (around the
    # piece's body center) to yield a useful alternative *placement*. True
    # for switches where IN-style and OUT-style placements are the same SKU
    # rotated end-for-end (LEGO sells 2 switch SKUs total — left + right —
    # not 4; IN/OUT is just orientation). Independent of `symmetric`: a
    # piece can be both rotatable and symmetric (flipping AND rotating both
