...
#   3=CROSS_90,
#   4=R40_SWITCH_LEFT, 5=R40_SWITCH_RIGHT,
#   6=DOUBLE_CROSSOVER
# IN vs OUT placement is encoded by the chromosome's rotate bit, not by
# separate piece IDs.
PIECE_COLORS = {
...
# Piece type constants for rendering (post-renumber)
...
<<<<<<< Updated upstream
    ...
    Keyword args:
        next_theta: Heading (degrees) of the FOLLOWING state along the walk.
...
<<<<<<< Updated upstream
        ...
        # in reverse (port B→A) at the cycle's branching slot, which makes
        # the next-state theta land on the curve's *entry* heading instead
        # of its exit — yielding delta=0 and a wrong-handed arc. The flip
        # bit is the ground truth for L vs R direction.
        flip = int(kwargs.get("flip", 0))
        angle = -CURVE_ANGLE if flip else CURVE_ANGLE
        _draw_curve_piece(ax, x0, y0, theta0, R40, angle, color, draw_rails_flag)
...
def _piece_state_map(layout: Layout) -> list:
    ...
    Handles per-component closing duplicates: if ``layout.component_breaks``
    is set, each component contributes its pieces to ``indices`` but its
    states get a trailing closing-duplicate, so piece i's state index
    skips past every closing duplicate that came before.

    For piece i in component k starting at piece-index ``c_k``:
      state_idx = i + k    (k closing duplicates inserted before)
      next_state_idx = state_idx + 1
    The component's last piece's ``next_state_idx`` lands on the closing
    duplicate, which equals the first piece's pose — so curves close
    correctly inside each component.
    """
    n = layout.n_pieces
    if n == 0:
        return []
    if layout.component_breaks is None or len(layout.component_breaks) <= 1:
        return [(i, min(i + 1, len(layout.states) - 1)) for i in range(n)]

    breaks = list(layout.component_breaks)
    breaks_with_end = breaks + [n]
    out: list = [(0, 0)] * n
    for k, start_piece in enumerate(breaks):
        end_piece = breaks_with_end[k + 1]
        state_offset = start_piece + k
        for j in range(end_piece - start_piece):
            piece_i = start_piece + j
            s = state_offset + j
            out[piece_i] = (s, s + 1)
    return out
...
<<<<<<< Updated upstream
        ...
        _draw_piece(ax, piece_idx, x[s], y[s], theta[s],
                    draw_rails_flag=True, next_theta=next_theta, flip=flip)
    ...
    for i in switch_positions:
        ...
        s, s_next = piece_state_map[i]
        color = get_piece_color(piece_idx)
        mid_x = (x[s] + x[s_next]) / 2
        mid_y = (y[s] + y[s_next]) / 2
    ...
    # Start = first piece of largest component; End = last piece of last component.
    s_first, _ = piece_state_map[0]
...
<<<<<<< Updated upstream
    ...
    # Closure metrics only meaningful for single-component layouts; for
    # multi-component the per-component closures are tracked separately and
    # the legacy ``closure_error`` is just states[0]→states[-1] (different
    # components — meaningless).
    if layout.n_components <= 1:
        metrics_lines.append(f"Closure Error: {layout.closure_error:.2f} studs")
